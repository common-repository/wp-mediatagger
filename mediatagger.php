...
//					(in_array($button, $button_list_disable) && $button_list_disable ? 'disabled' : '') . ' />';
	...
	private function multisort_insert($result_page_url='', $num_tags_displayed = '', $font_size_min = '', $font_size_max = '', 
		...
		$p_search = isset($_POST['search']) ? $_POST['search'] : NULL;
		$p_mdtg_tags = isset($_POST['mdtg_tags']) ? $_POST['mdtg_tags'] : NULL;
		$p_free_search = isset($_POST['free_search']) ? $_POST['free_search'] : NULL;
		$p_last_free_search = isset($_POST['last_free_search']) ? $_POST['last_free_search'] : NULL;
		$p_link_triggered = isset($_POST['link_triggered']) ? $_POST['link_triggered'] : NULL;
		$p_search = isset($_POST['search']) ? $_POST['search'] : NULL;
		$p_search_mode = isset($_POST['search_mode']) ? $_POST['search_mode'] : NULL;
		$p_coming_from_widget = isset($_POST['coming_from_widget']) ? $_POST['coming_from_widget'] : NULL;
		
		// $tax_id_list = (isset($tax_id_list) ? $tax_id_list : ($_POST['search'] == "Clear" ? array() : $_POST['mdtg_tags'])); 
		// changed to line below to avoid NOTICE messages in log file
		$tax_id_list = (isset($tax_id_list) ? $tax_id_list : ($p_search == "Clear" ? array() : $p_mdtg_tags)); 
		...
		$free_search = (($p_search == "Clear" || ($p_free_search == $p_last_free_search) && ($p_link_triggered<21)) ? "" : $p_free_search);
		...
		$search_mode = ($called_from_widget ? 1 : (isset($search_mode) ? $search_mode : 
			( $p_coming_from_widget ? $preset_search_mode : ($p_search_mode ? $p_search_mode : $preset_search_mode))));	
		...
		$change_page_previous = 0;
		$change_page_next = 0;
	
		switch ($p_link_triggered){
			...
			case 21:
			case 22:
			case 23: $result_mode = $p_link_triggered - 20; break; 
		...
		$p_tagcloud = isset($_POST['tagcloud']) ? $_POST['tagcloud'] : NULL;
		if ($p_tagcloud > 0) {
			unset($tax_id_list);
			$tax_id_list[0] = $p_tagcloud;
		}
...
/*		
		if (mysql_error()) {	///// BB 4/6/2017 - section below replaced due to incompatibility with PHP7
...
*/
		if ($wpdb->last_error) {
			echo 'MYSQL query returned error executing query <strong>"'. $sql_query . '"</strong> : <br/>=> <span style="color:red">' . 
				htmlentities($wpdb->last_error) . '</span><br/>';
			$sql_result = "SQL_EXEC_ERROR";
		}
		return $sql_result;
	}
